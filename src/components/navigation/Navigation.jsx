      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {/* Mobile Auth */}
            <div className="p-4 border-b border-[rgba(196,163,90,0.2)]">
...
function MobileAuth({ onClose }) {
    ...
    return (
      <div className="flex items-center gap-3">
        <img
          ...
          alt="Avatar"
          className="w-10 h-10 rounded-full border border-[var(--color-gold)]/30"
        />
        <div className="flex-1 min-w-0">
          <p className="text-sm text-[var(--color-gold)] font-medium truncate">{user.user_metadata?.full_name}</p>
          <button
            onClick={() => { signOut(); onClose(); }}
            className="text-xs text-[var(--color-mist)] hover:text-[var(--color-vermillion)] transition-colors"
          >
            Đăng xuất
          </button>
        </div>
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[var(--color-gold)]/10 text-[var(--color-gold)] border border-[var(--color-gold)]/30 rounded-[var(--radius-lg)] hover:bg-[var(--color-gold)]/20 transition-all font-medium"
    >
      <User size={18} />
              ...
              user ? (
                <div className="flex items-center gap-3">
                  <div className="text-right hidden xl:block">
                    <p className="text-sm text-[var(--color-gold)] font-medium">{user.user_metadata?.full_name || user.email}</p>
                    <p className="text-xs text-[var(--color-mist)]">Thành viên</p>
                  </div>
                  <img
                    src={user.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${user.email}&background=random`}
                    alt="Avatar"
                    className="w-8 h-8 rounded-full border border-[var(--color-gold)]/30"
                  />
                  <button
                    onClick={signOut}
                    className="text-xs text-[var(--color-mist)] hover:text-[var(--color-vermillion)] transition-colors"
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-[var(--color-gold)]/10 text-[var(--color-gold)] border border-[var(--color-gold)]/30 rounded-full hover:bg-[var(--color-gold)]/20 transition-all font-medium text-sm"
                >
                  <User size={16} />
                  Đăng nhập
                </button>
